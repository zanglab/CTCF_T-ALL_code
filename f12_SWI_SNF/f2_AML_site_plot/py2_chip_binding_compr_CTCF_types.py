...
def plot_composite_figs(df,aml_gained,aml_lost,constitutive,outdir,basename,window):
    ...
    half_window = 3
    x = df.columns
    y1 = df.loc[aml_gained.index];y1 = window_smooth(y1,half_window)
    y2 = df.loc[aml_lost.index];y2 = window_smooth(y2,half_window)
#     y3 = df.loc[selected_ctrl];y3 = window_smooth(y3,half_window)
    y4 = df.loc[constitutive.index];y4 = window_smooth(y4,half_window)
    fig = plt.figure(figsize=(2.8,2.8))
    plt.plot(x,y1.mean().values,label='AML gained',color='red')
    plt.plot(x,y2.mean().values,label='AML lost',color='blue')
...
#     sns.despine(offset=0, trim=False)
    ...
    plt.xlabel('position relative to CTCF',fontsize=16 )
...
def main():
    ...
    aml_gained,aml_lost = CTCF_TALL_modules_new.return_cancer_specific_binding('AML')
