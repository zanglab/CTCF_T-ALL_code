...
def read_tag_info(peak_results,treatment):
    ...
    for line in inf.readlines():
        if line.startswith('# total tags in {}: '.format(treatment)):
            mapped_reads = int(line.strip().split('# total tags in {}: '.format(treatment))[1])
        if line.startswith('# tags after filtering in {}: '.format(treatment)):
            uniq_reads = int(line.strip().split('# tags after filtering in {}: '.format(treatment))[1])
    inf.close() 
    ...
    if len(block)>0:
        ...
        if outname==blockname:
            ...
            for fqname in treatnames:
                total_reads = write_fq_process_qc(outname,fqname,fqdir,outdir,species,split_flag)
