...
def bar_plot_enrichment(vals,xticklabels,figname):
    ...
    if p<0.05:
        ...
        if p<0.001:
            star_mark="**"
    plt.text(plot_positions[1] , plot_vals[1]*1.02 , star_mark ,ha = 'center',fontsize=16)
    ...
    plt.text(plot_positions[2] , plot_vals[2]*1.02 , star_mark ,ha = 'center',fontsize=16)
    plt.axes().set_xticks(plot_positions)
    plt.axes().set_xticklabels(xticklabels,rotation=25,ha = 'right',fontsize=15)
...
def main(infile):
    ...
    figname = '{}/CUTLL1_shCTCF_Dn_enrichment_bar.pdf'.format(outdir)
    xticklabels=['All genes','T-ALL gained intra-domain','T-ALL gained targets']
