...
def return_CTCF_state_df(cancertype,prename,flag):
    ...
    # ==== methylation data to rerank the index        
...
def combine_plot(cancertype,flag,outdir,satuation,vmin,vmax):
    ...
    for pos_id in np.arange(total_len):
        ...
        loc_df.to_csv(outdir+os.sep+'{}_{}.csv'.format(flag,prenames[pos_id]))
        
        sub_heatmap_plot(loc_df,gs,pos_id,titles[pos_id],total_len,satuation,vmin,vmax,flag,cancertype)
#     plt.savefig(outdir+os.sep+'{}_CTCF_{}.pdf'.format(flag,cancertype),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
    plt.savefig(outdir+'/figs/{}_CTCF_{}.png'.format(flag,cancertype),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
    plt.close();
...
def main():
    ...
    outdir = 'plot_data_CTCF_heatmap_rerank_by_methylation'
    os.makedirs(outdir,exist_ok=True)
    os.makedirs(outdir+os.sep+'figs',exist_ok=True)

    cancertypes=['T-ALL','BRCA','CRC','LUAD','PRAD_TissueAdded']
