...
def sub_heatmap_plot(df,gs,loc,title,total,satuation,vmin,vmax,flag,cancertype):
    ...
    if loc==0:   
        ...
        ax.set_ylabel('CTCF binding'.format(flag.capitalize()),fontsize=18)
...
def plot_methylation_bar_old(df,gs,loc):
...
#     df = df[columns]
    ...
    for ii in np.arange(len(df.index)):
        ...
        if df.loc[df.index[ii],columns[1]]>=diff_CpG_thre_cov:
            ...
            g2 = ax.barh(ii,-100,color='silver',height=1,lw=0)
            g2 = ax.barh(ii,100,color='silver',height=1,lw=0)
...
#     ax.set_ylabel('$\Delta$(DNA methylation)',va='baseline',fontsize=18)
    ax.set_title('$\Delta$DNAme',fontsize=17,x=.7)
    ax.spines['left'].set_visible(True)
    ax.spines['right'].set_visible(True)
    ax.text(110,df.shape[0]-1,'{:4}'.format(df.shape[0]),fontsize=14,ha='left')
...
def plot_methylation_bar(df,gs,loc,cancertype,flag):
    ...
    ax.set_yticks([])
    ax.set_xticks([])
#     ax.set_title('$\Delta$DNAme',fontsize=18,x=.7)
    ax.set_title('$\Delta$ DNA\n    methylation',fontsize=18,x=.7)

    # ==== add additional title
    add_title='{} {} sites (n={})'.format(cancertype.split('_')[0],flag,df.shape[0])
    ax.text(-600,-0.3*ii-.5,add_title,fontsize=18)    
...
#     ax.text(115,df.shape[0]-1,'{:4}'.format(df.shape[0]),fontsize=15,ha='left')
    return df, df1.index, df2.index, df3.index
...
cancertype_title_list = \
    {'T-ALL_patient1':{'methy_matches':"T-ALL_patient1",'chip_matches':['T-ALL_p1_T-ALL_normal','T-ALL_p1_PD9'],'titles':['T cell','T-ALL patient1']},\
     'T-ALL_patient2':{'methy_matches':"T-ALL_patient2",'chip_matches':['T-ALL_p2_T-ALL_normal','T-ALL_p2_PD31'],'titles':['T cell','T-ALL patient2']}}


outdir = 'patients_fig_DNAme'
os.makedirs(outdir,exist_ok=True)

cancertypes = ["T-ALL_patient1","T-ALL_patient2"]
...
for bindingtype in bindingtypes:
    for cancertype in cancertypes:
        ...
        normal_file = "data/plot_data_CTCF_heatmap_rerank_by_methylation_patients/{}_{}.csv".format(bindingtype,cancertype_title_list[cancertype]['chip_matches'][0])
        cancer_file = "data/plot_data_CTCF_heatmap_rerank_by_methylation_patients/{}_{}.csv".format(bindingtype,cancertype_title_list[cancertype]['chip_matches'][1])
...
#         methy_df = pd.read_csv(methylation_file,index_col=0)
#         grey_index = methy_df[methy_df['DE_count_EachSide150bp']<3].index
        ...
        re_index_df, df1_index, df2_index, df3_index = plot_methylation_bar(df,gs,loc,cancertype,bindingtype)
#         re_index_df.to_csv(outdir+'/csv/{}_{}_methylation.csv'.format(bindingtype,cancertype))
        ...
        re_index = [i for i in df.index if i in df1_index] + [i for i in df.index if i in df2_index]+[i for i in df.index if i in df3_index]
        df = df.reindex(re_index)
