...
def plot_mutation_changes(df,gs,loc,cancertype):    
...
#     plt.axes().text(xleft,df.shape[0]*1.3,'-',ha='center',fontsize=20)
    ...
    ax.set_yticks([])
    ax.set_xticks([])
    ax.set_title('$\Delta$motif-score',va='baseline',fontsize=18,x=0.65)

    # ==== add additional title
#     ax.text(-36,-0.17*ii-.5,cancertype.split('_')[0],fontsize=18)    
#     ax.hlines(y=-0.15*ii-.5,xmin=-75,xmax=12,clip_on=False,lw=1.1)
...
def sub_heatmap_plot(df,gs,loc,title,total,satuation,vmin,vmax,flag):
...
#     ax.axvlines(0,ymax=bar_pos,c='k',lw=1.1)
...
cancertype_title_list = \
    {'Jurkat':	{'methy_matches':"T-ALL_1",	'mutation_matches':"Jurkat", 'ctcf':['T-ALL_normal','T-ALL_cancer_JURKAT']},\
     'CUTLL1':	{'methy_matches':"T-ALL_1",	'mutation_matches':"CUTLL1", 'ctcf':['T-ALL_normal','T-ALL_cancer_CUTLL1']},\
     'T-ALL patient1':{'methy_matches':"T-ALL_1",	'mutation_matches':"PD9", 'ctcf':['T-ALL_normal','PD9']},\
     'T-ALL patient2':{'methy_matches':"T-ALL_1",	'mutation_matches':"PD30", 'ctcf':['T-ALL_normal','PD31']}}


outdir = 'patients_fig_mutation'
os.makedirs(outdir,exist_ok=True)
os.makedirs(outdir+os.sep+'csv',exist_ok=True)

cancertypes = ["Jurkat","CUTLL1","T-ALL patient1","T-ALL patient2"]


bindingtypes = ["gained","lost"]
satuation,vmin,vmax = 98,0,100

ctcf_binding_dir='/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/updated_201906/f3_cancer_specific_CTCFs/f8_panCancer_CTCF_binding_pattern/f3_cancerType_CTCF_heatmap/f1_gained_lost_CTCF_binding'
...
#         cancer_file = "data/plot_data_CTCF_heatmap_rerank_by_methylation/{}_{}_cancer.csv".format(bindingtype,cancertype)
        normal_file = ctcf_binding_dir+os.sep+"{}_{}.csv".format(bindingtype,cancertype_title_list[cancertype]['ctcf'][0])
        cancer_file = ctcf_binding_dir+os.sep+"{}_{}.csv".format(bindingtype,cancertype_title_list[cancertype]['ctcf'][1])
        mutation_file = "data/plot_data_mutation_patients/{}_{}.csv".format(bindingtype,cancertype_title_list[cancertype]['mutation_matches'])     
        ...
        # ====re-rank the index, if exists matched DNA me. file
        if os.path.isfile(methylation_file):
            df_tmp = pd.read_csv(methylation_file,index_col=0)
            re_index_df = plot_methylation_bar(df_tmp,gs,loc)
        else:
            re_index_df = df

        df = df.reindex(re_index_df.index)
#         df.to_csv(outdir+'/csv/{}_{}_CTCF_normal.csv'.format(bindingtype,cancertype))
        sub_heatmap_plot(df,gs,loc,'T cell',total,satuation,vmin,vmax,bindingtype)
...
#         df.to_csv(outdir+'/csv/{}_{}_CTCF_cancer.csv'.format(bindingtype,cancertype))
        sub_heatmap_plot(df,gs,loc,'\n'.join(cancertype.split(' ')),total,satuation,vmin,vmax,bindingtype)

        
        loc = 3
        df = pd.read_csv(mutation_file,index_col=0)        
        df = df.reindex(re_index_df.index)
        plot_mutation_changes(df,gs,loc,cancertype)
