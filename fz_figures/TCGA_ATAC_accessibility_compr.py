...
def mark_pvalue(compr_pos,positions,box_vals):
    ...
    y, h, col = np.percentile(np.append(box_vals[compr_pos[0]],box_vals[compr_pos[1]]),100)*0.85 ,1.1, 'k'
    y2 = np.percentile(np.append(box_vals[compr_pos[0]],box_vals[compr_pos[1]]),0)*0.85
...
#             p_label = '**'
        if compr_pos[2] == 't':
            ...
            plt.text((x1+x2)*.5, y2*1.2, p_label, ha='center', va='top', color=col,fontsize=14)
...
def main():
    ...
    outdir = 'figures/TCGA_ATAC_accessibility_compr'
    os.makedirs(outdir,exist_ok=True)
