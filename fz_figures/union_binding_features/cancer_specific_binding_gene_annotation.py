...
def compr_plot_box(annotation,flag,outdir):
    ...
    if flag=='lost':
        plt.ylabel('CTCF sites (%)'.format(flag),fontsize=16)
    plt.ylim([0,100])
    plt.xlim([0.3,6.7])
    plt.title('{} sites'.format(flag.capitalize()),fontsize=18)
    if flag=='gained':
        plt.legend([g3,g2,g1,g0],["Promoter","Exon","Intron","Intergenic"],loc=1,bbox_to_anchor=(1.7,1.03),fontsize=16,borderaxespad=0.,labelspacing=.2,handletextpad=0.2,handlelength=1,frameon=False)
    ...
    plt.axes().set_xticklabels(ticklabels,rotation=45,ha='right',fontsize=16) 
...
def main():
    ...
    outdir = 'figures/cancer_specific_binding_gene_annotation'
