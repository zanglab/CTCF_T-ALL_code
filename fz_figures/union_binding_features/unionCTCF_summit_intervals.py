...
# ==== ori dir and files ===
ori_dir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/updated_201906/f2_union_CTCFs/f1_union_all_summits/'
outdir = 'figures'
...
def main(infile):               
    ...
    subintervals = [int(line.strip()) for line in open('{}/f1_peak2000_datasets_union_summits/all_summits_fe4_intervals.txt'.format(ori_dir)).readlines()]
    ...
    plt.yscale('log')
    plt.ylabel('Number of adjacent sites')
    plt.xlabel('distance between adjacent sites (bp)')
    plt.savefig('{}/unionCTCF_summit_intervals.pdf'.format(outdir),bbox_inches='tight',pad_inches=0.02,transparent=True)
    plt.close()
