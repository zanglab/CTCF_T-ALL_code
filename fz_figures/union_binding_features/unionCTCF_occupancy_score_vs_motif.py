...
def plot_occurrene_vs_motif(df):
    ...
    a1 = ax1.plot(df.index,df['#bindings'],label='site count',color='navy')
    ax1.set_yscale('log')
    ax1.set_ylabel('Number of CTCF sites',fontsize=16)
    ax1.set_xlabel('Occupancy score',fontsize=16)
    ax1.set_xticks([0,250,500,750])
    #ax1.legend(bbox_to_anchor=(1.2,1))

    ax2 = ax1.twinx()
    a2 = ax2.plot(df.index,100*df['%motif'],label='sites with motif',color = 'coral')
    #ax2.legend(bbox_to_anchor=(1.2,0.7))

    ls = a1+a2
    labs = [l.get_label() for l in ls]
    ax1.legend(ls,labs,frameon=False,bbox_to_anchor=(.9,.24),fontsize=14,borderaxespad=.2,labelspacing=.2,handletextpad=0.5,handlelength=1.5)
    ...
    ax2.set_ylabel('sites with motif (%)',fontsize=16)
