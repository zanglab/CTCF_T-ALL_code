...
def main():
    ...
    outdir = 'figures/cancer_specific_binding_overlap'
    ...
    g.set_xticklabels(ticklabels,rotation=45,ha='left',fontsize=16)
    ...
    for x in range(0,len(cancertypes)):
        for y in range(0,len(cancertypes)-x):
            plt.text(x + 0.5 , len(cancertypes)-y - 0.7, int(gained_df_raw.loc[gained_df.index[x],gained_df.columns[len(cancertypes)-1-y]]),fontsize=11,ha='center') #data[y,x] +0.05 , data[y,x] + 0.05
#     plt.text(-2,7.,'Gained sites',fontsize=18)
#     plt.axes().axhline(y=0, color='grey',linewidth=2)
#     plt.axes().axvline(x=0, color='grey',linewidth=2)
    plt.axes().invert_yaxis()
    plt.savefig('{}/panCancer_gained_CTCF_overlapped_jaccard.pdf'.format(outdir),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
    plt.close()
    ...
    g.set_xticklabels(ticklabels,rotation=45,ha='right',fontsize=15)
    g.set_yticklabels(ticklabels,rotation=0,ha='left',fontsize=15)
    plt.axes().tick_params(axis='x',direction='out', length=0, width=1, colors='black')
    plt.axes().tick_params(axis='y',direction='out', length=0, width=1, colors='black')
    g.yaxis.set_ticks_position('right') 
    for x in range(lost_df.shape[1]):
        for y in range(len(cancertypes)-1-x,lost_df.shape[1]):
            plt.text(x + 0.1 , len(cancertypes)-y - .7, int(lost_df_raw.loc[lost_df.index[x],lost_df.columns[len(cancertypes)-1-y]]),fontsize=11,ha='left') #data[y,x] +0.05 , data[y,x] + 0.05
    #cbar = g.collections[0].colorbar
    #cbar.set_ticks([0,4])
    #cbar.set_ticklabels([0,1])
#     plt.axes().axhline(y=4, color='grey',linewidth=2)
#     plt.axes().axvline(x=4, color='grey',linewidth=2)
#     plt.text(6,-2,'Lost sites',fontsize=18)
    plt.axes().invert_yaxis()
    plt.savefig('{}/panCancer_lost_CTCF_overlapped_jaccard.pdf'.format(outdir),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
    plt.close()
