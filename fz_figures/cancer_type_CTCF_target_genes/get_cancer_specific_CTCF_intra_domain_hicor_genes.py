    ...
    for cancertype in cancertypes:
        for binding_type in ['gained','lost']:
            for suf_name in suf_names:
                ...
                domain_upgenes = [j for i in info_df['domain_upgenes_hicor_names'].dropna().values for j in i.split(',')]
                domain_downgenes = [j for i in info_df['domain_dngenes_hicor_names'].dropna().values for j in i.split(',')]
                
                with open(outdir+os.sep+'{}_{}_up_hicor_genes_{}.txt'.format(cancertype,binding_type,suf_name),'w') as outf:
                    outf.write('\n'.join(domain_upgenes)+'\n')
                with open(outdir+os.sep+'{}_{}_down_hicor_genes_{}.txt'.format(cancertype,binding_type,suf_name),'w') as outf:
