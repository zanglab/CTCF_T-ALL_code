...
def sub_heatmap_plot(df,gs,loc,title,total,satuation,vmin,vmax,flag,cancertype):
    ...
    if loc==0:   
        ...
        ax.set_ylabel('ChIP-seq'.format(flag.capitalize()),fontsize=16)
        # only show the first/last xtick
        xp = g.get_xticks()
        ax.set_xticks([xp[0],xp[-1]])
        ax.set_xticklabels(['',''],rotation=30,fontsize=16)
    ...
    # ==== show #bindings near the last heat map
        ...
        ax.set_ylabel('')
#         if loc==total-1:
#             ax.text(300,df.shape[0],'{}'.format(df.shape[0]),fontsize=16,ha='right')
        


def combine_plot(dfs,re_index,titles,cancertype,flag,outdir,satuation,vmin,vmax):
    # for each cancer type, return cancer/normal df for heatmap
    total_len = len(dfs)
    fig = plt.figure(figsize = (5.2,2.7))
    width_ratio = [1]*total_len
    gs = gridspec.GridSpec(1,total_len,width_ratios=width_ratio,wspace=0.0) 
    for pos_id in np.arange(total_len):
        loc_df = dfs[pos_id]
        loc_df = loc_df.reindex(re_index)
        sub_heatmap_plot(loc_df,gs,pos_id,titles[pos_id],total_len,satuation,vmin,vmax,flag,cancertype)
    plt.savefig(outdir+os.sep+'{}_CTCF_with_SMC3_{}.pdf'.format(flag,cancertype),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
    plt.close();
...
def return_CTCF_state_df(cancertype,prename,flag):
    ...
    binding_file = binding_dir+os.sep+'{}_{}_{}.csv'.format(flag,cancertype,prename)
...
def return_smc3_state_df(cancertype,prename,flag):
    # return the df for heatmap
    binding_dir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/updated_201906/fz_figures/ctcf_TALL_with_SMC3/f2_T_ALL_gained_lost_SMC3_binding'
    binding_file = binding_dir+os.sep+'{}_{}_{}.csv'.format(prename,cancertype,flag)
    with open(binding_file) as binding_inf:
        binding_df = pd.read_csv(binding_inf,index_col=0)
    binding_df = window_cumulative(binding_df)
#     binding_df = signal_centered(binding_df)
...
def main():
    ...
    outdir = 'f3_CTCF_heatmap_TALL_vs_CD4_add_SMC3'
    ...
    for cancertype in cancertypes:
        ...
        for satuation in [98]:
            for vmax in [120]:
                ...
                flag = 'gained'
                ctcf_Tcell = return_CTCF_state_df(cancertype,'normal',flag)
                ctcf_jurkat = return_CTCF_state_df(cancertype,'cancer_JURKAT',flag)
                ctcf_cutll1 = return_CTCF_state_df(cancertype,'cancer_CUTLL1',flag)
                smc3_df = return_smc3_state_df(cancertype,'CUTLL1_SMC3_on',flag)
                dfs = [ctcf_Tcell,ctcf_jurkat,ctcf_cutll1,smc3_df]
                re_index = (ctcf_jurkat+ctcf_cutll1).mean(axis=1).sort_values(ascending=False).index
                combine_plot(dfs,re_index,titles,cancertype,flag,outdir,satuation,vmin,vmax)#;exit()
#                 combine_plot(cancertype,'lost',outdir,satuation,vmin,vmax)
                ...
                smc3_df = return_smc3_state_df(cancertype,'CUTLL1_SMC3_on',flag)
                dfs = [ctcf_Tcell,ctcf_jurkat,ctcf_cutll1,smc3_df]
                re_index = ctcf_Tcell.mean(axis=1).sort_values(ascending=False).index
                combine_plot(dfs,re_index,titles,cancertype,flag,outdir,satuation,vmin,vmax)#;exit()
