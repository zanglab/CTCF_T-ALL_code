...
def plot_num_data(df,columns,cancertype,outdir,bins=20,xrange=False,yrange=False,logscale=True,base=0.1):
    ...
    figname = outdir+os.sep+'{}_vs_others_occurrence'.format(cancertype)
    if cancertype=="PRAD_TissueAdded":
        cancertype="PRAD"
    xlabel='Occupancy score in\n {} {} datasets'.format(xrange[1]-1,cancertype)
    ylabel='Occupancy frequency in\n other {} datasets'.format(yrange[1]-1)
    title = cancertype
...
# 
#     plt.figure(figsize=return_fig_size(cancertype))
# 
#     if logscale:
...
# 
#     plt.axes().set_title(title,fontsize=19)
#     plt.axes().set_xlabel(ylabel,fontsize=17)
#     plt.axes().set_ylabel(xlabel,fontsize=17)
#     
...
def main():
    ...
    outdir = 'f3_cancer_normal_occurrence_csv'
    ...
    for cancertype in cancertypes:
        ...
        with open(co_occurrence_file) as co_occurrence_inf:
            ...
            plot_num_data(df,columns,cancertype,outdir,bins=10,xrange=xrange,yrange=yrange)
