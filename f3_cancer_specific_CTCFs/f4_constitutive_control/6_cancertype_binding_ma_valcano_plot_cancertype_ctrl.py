...
def ma_fit(df_ori,columns):
    ...
    df['log2_avg']=0.5*(df[c]+df[t])
    df['log2FC']=df[t]-df[c] 
    
    ## M=kA+C
    a,b,xmean,ymean = linear_regression(df['log2_avg'].values,df['log2FC'].values)
    print(a,b)
    df['reg_log2FC'] = a*df['log2_avg']+b
    df['adj_log2FC'] = df['log2FC']-df['reg_log2FC']
...
def gained_lost_scatter(x,y,figname,cancertype,xlabel,ylabel,hline):
    ...
    #const_df = pd.read_csv('/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/12_new_gained_lost_from_all_features/f2_bindings_signals_fdr/f3_proper_ctrl_constitutive_selection/const_NONoverlapped_panCancer_fdr_LT_001.bed',sep='\t',index_col=3,header=None)
...
def cancertype_maplot(cancertype,compr_type):
    ...
    #gained_lost_scatter(x,y,figname,cancertype,xlabel,ylabel,hline=-np.log10(0.01))
