...
def survival_for_two(df,treat,ctrl,legends,title,figname):
    ...
    lg = ax.legend(handles[1::2], legends,loc='lower left',borderaxespad=-.15,handletextpad=.2,labelspacing=.1,handlelength=1.1)
    if pvalue<0.05:
...
def return_survival_df_50th_by_mean(clinical_df):

    # get the clinical info, using top/bottom 25% as cutoff
    clinical_df = clinical_df.sort_values(by=['diff_score_mean'],ascending=False)
...
def return_survival_df_50th_by_median(clinical_df):

    # get the clinical info, using top/bottom 25% as cutoff
    clinical_df = clinical_df.sort_values(by=['diff_score_median'],ascending=False)
    a_index = clinical_df.index[:int(len(clinical_df.index)*.5)]
    b_index = clinical_df.index[-1*int(len(clinical_df.index)*.5):]
    print('treat:\t',len(a_index),'\nctrl:\t',len(b_index))
    df = pd.DataFrame(index = a_index.union(b_index))
...
def main():
    ...
    name_types = ['BRCA_basicCTCF_overlapped_Breast','COAD_basicCTCF_overlapped_Colon','LUAD_basicCTCF_overlapped_Lung']
    name_types_cancer_match = {'BRCA_basicCTCF_overlapped_Breast':'BRCA','COAD_basicCTCF_overlapped_Colon':'CRC','LUAD_basicCTCF_overlapped_Lung':'LUAD'}

    for name_type in name_types:
        combined_file = combined_file_dir+os.sep+'{}_lost_diff_score_mean_median_with_clinical_info.csv'.format(name_type)
        ...
        df = return_survival_df_50th_by_mean(clinical_df)
        df.to_csv(outdir+os.sep+'{}_survival_info_by_mean.csv'.format(name_type))
        figname=outdir+os.sep+'{}_survival_50th_by_mean.png'.format(name_type)
        survival_for_two(df,treat,ctrl,legends,title,figname)

        # rank by median
        df = return_survival_df_50th_by_median(clinical_df)
        df.to_csv(outdir+os.sep+'{}_survival_info_by_median.csv'.format(name_type))
        figname=outdir+os.sep+'{}_survival_50th_by_median.png'.format(name_type)
        survival_for_two(df,treat,ctrl,legends,title,figname)
