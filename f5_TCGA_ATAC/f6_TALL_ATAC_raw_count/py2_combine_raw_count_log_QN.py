...
def main():
    ...
    df = df.round(2)     
    df.to_csv(outdir+os.sep+'Jurkat_CD4_ATAC_combined_raw_count.csv')
    
    df = np.log2(df+5)
    df = quantile_normalization(df) 
    df = df.round(2)     
    df.to_csv(outdir+os.sep+'Jurkat_CD4_ATAC_combined_raw_count_log2_QN.csv')
