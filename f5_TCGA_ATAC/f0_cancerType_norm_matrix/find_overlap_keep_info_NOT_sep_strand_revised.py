...
def read_region_from_bed(infile,species,expand=0,write_out=None,end_col=2,region_sorted=True):
    ...
    with open(infile,'r') as inf:
        ...
        while line:
            #print(line)
            if not (re.match("#",line) or (re.match("seq",line))):
                #print(line)
    ...
    return regions1,regions2,head_line


def write_regions_bed3(regions1,regions2,outfile,head_line1):
    # write the regions(with chrom,start,end info) into bed3 format
    with open(outfile,'w') as outf:
        if head_line1:
            outf.write(head_line1)
        for chrom in regions1:
            for region in regions1[chrom]:
                outf.write('{}\t{}\t{}\t{}\n'.format(chrom,region[0],region[1],region[2]))
        for chrom in regions2:
...
def main(infile1,infile2,species,outfile1,outfile2,expand1,expand2):

    compare_regions1,compare_regions2,head_line1 = read_region_from_bed(infile1,species,expand1)
    basic_regions1,basic_regions2,head_line2 = read_region_from_bed(infile2,species,expand2)
    #print(compare_regions1,'a');exit()
    ...
    #print(basic_regions1);exit()
    overlapped1,nonoverlapped1 = get_overlap_info_strID(compare_regions1,basic_regions1)        
    overlapped2,nonoverlapped2 = get_overlap_info_strID(compare_regions2,basic_regions2)
    if outfile1:
        write_regions_bed3(overlapped1,overlapped2,outfile1,head_line1) 
    if outfile2:
        write_regions_bed3(nonoverlapped1,nonoverlapped2,outfile2,head_line1)         
...
if __name__ == '__main__':
    ...
    main(args.infile1,args.infile2,args.species,args.outfile1,args.outfile2,args.expand1,args.expand2)
