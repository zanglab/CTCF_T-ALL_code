...
def compr_plot_box(gained_df,lost_df,const_df,cancer_columns,ctrl_columns,outdir,cancertype,flag):
    ...
    #plt.ylabel('ATAC-seq \ndifferential score \n{} over others'.format(cancertype),fontsize=16)
...
#     sns.despine(offset=None, trim=False)
    ...
    tcga_cancer = cancertype_match_names[cancertype]
    plt.ylabel('$\Delta$ (ATAC-seq score)\n{} over others'.format(tcga_cancer),fontsize=16)
#     plt.title('{}'.format(title),fontsize=16,va='baseline')
    plt.axes().set_xticklabels(['{} lost'.format(cancertype.split('_')[0]),'Constitutive','{} gained'.format(cancertype.split('_')[0])],rotation=30,ha='right',fontsize=16) 
    plt.savefig('{}/{}_ATAC_sig_compr.pdf'.format(outdir,cancertype),bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
...
def main():
    ...
    indir = 'f1_ATAC_sig_abstract'
    outdir = 'f3_sig_compr_layout_fc'
    os.makedirs(outdir,exist_ok=True)
