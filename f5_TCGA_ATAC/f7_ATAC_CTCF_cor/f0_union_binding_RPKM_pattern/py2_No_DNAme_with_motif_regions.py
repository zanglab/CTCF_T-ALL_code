...
def main():    
    ...
    methylated_df = pd.read_csv('data/Jurkat_DNA_methy_status_GT15.csv',index_col=0)    
#     methylated_df = pd.read_csv('Jurkat_DNA_methy_status_GT15_CpGs_GT3.csv',index_col=0)
    ...
    union_df.to_csv('data/union_CTCF_No_Jurkat_DNAmethylation_with_motif.csv')
    union_df.insert(3,'id',union_df.index)
    union_df.to_csv('data/union_CTCF_No_Jurkat_DNAmethylation_with_motif.bed',sep='\t',header=None,index=False)
