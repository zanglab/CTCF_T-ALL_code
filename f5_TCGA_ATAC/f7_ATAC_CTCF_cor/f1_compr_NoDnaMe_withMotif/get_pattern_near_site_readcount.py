...
def generate_patterns(site_positions,tagsfile,species,format,fragmentsize,width,bins,expand,mid):
...
def main(sitefile,tagsfile,outfile,species,format,fragmentsize,width,bins,expand,mid):
    ...
    site_positions.columns = ['chr','position','end','id']+[i for i in np.arange(site_positions.shape[1]-6)]
    patterns = generate_patterns(site_positions,tagsfile,species,format,fragmentsize,width,bins,expand,mid)
