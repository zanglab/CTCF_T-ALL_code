    ...
    for chrom in islands:
        if chrom not in regions:
            ...
            for island in islands[chrom]:              
                ...
                if s > e:
                    ### position columns
                    #col_poses = [int(regions[chrom][i][0]*0.5+regions[chrom][i][1]*0.5) for i in np.arange(e,s)]#  HERE IS THE ADDED COLUMN
                    #col_info = ','.join(map(str,col_poses))#  HERE IS THE ADDED COLUMN
                    ### info columns
                    col_info = ','.join(regions[chrom][i][2].split()[0] for i in np.arange(e,s))#  HERE IS THE ADDED COLUMN
                    island[2] = island[2]+'\t'+col_info#;print(island,regions[chrom][e:s],col_info);exit()
...
def return_udhs_overlapped_ids(infile1,infile2,species,outfile1=None,expand1=0,expand2=0):
    # infile2 should be udhs file
    ...
    return overlapped1,nonoverlapped1
