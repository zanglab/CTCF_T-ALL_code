...
def scatter_plot(df,marked_index,figname,mark_col):
    ...
    for marker_id in sorted_marker_pos:
        ...
        values = -1*np.log10(df.loc[df.index[marker_id],mark_col])-values_reset#;print(values)
...
def main_plot(infile,outdir_suffix):

    outdir = 'rank_dot_figs_{}'.format(outdir_suffix)
    ...
    df[mark_col]=df[mark_col]+1e-300
...
def main(infile):
    ...
    infiles = glob.glob('TF_enrichemnt/*fisher.csv')
    for infile in infiles:
        main_plot(infile,'TF_enrich')   


    infiles = glob.glob('TF_enrichemnt_with_control/*fisher.csv')
    for infile in infiles:
        main_plot(infile,'TF_enrich_with_control')   
