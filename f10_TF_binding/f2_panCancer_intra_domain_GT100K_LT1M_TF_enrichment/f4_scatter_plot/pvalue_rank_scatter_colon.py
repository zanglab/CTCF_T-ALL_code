...
def scatter_plot(df,marked_index,figname,mark_col):
    ...
    p_seudo=1E-308
    df[mark_col]=df[mark_col]+p_seudo
    ...
    sorted_marker_ids = np.append(df.index[:3],df.index[:200].intersection(marked_index)[:5])
...
#             plt.text(xp+120+x_pos_m,values_reset,df.index[marker_id],color='k')
        # ==== plt.arrow(x,y,dx,dy)
...
def main(infile):
    ...
    marked_index = ['TEAD4','SMC1A','SNAI2','CDCA2']
