...
def mark_pvalue(compr_pos,positions,box_vals,flag):
    ...
    if p<0.05:
        ...
        if compr_pos[2] == 't':
            ...
            plt.text((x1+x2)*.5, y*1.0, p_label, ha='center', color=col,fontsize=15)
        else:
...
def main():
    ...
    for basename in ['T-ALL_gained','T-ALL_lost']:
        for hic_normalization in ['raw']:
            a6010_score_file = 'f2_dynamic_CTCF_Notch1_hic_interaction/{}_ctcf_notch_interaction_vs_bg_in_a6010_{}.csv'.format(basename,hic_normalization)
            jurkat_score_file = 'f2_dynamic_CTCF_Notch1_hic_interaction/{}_ctcf_notch_interaction_vs_bg_in_jurkat_{}.csv'.format(basename,hic_normalization)          
            print(basename,hic_normalization,os.path.isfile(a6010_score_file),os.path.isfile(jurkat_score_file))
            a_zsocres = return_notch_bg_scores(a6010_score_file)
            j_zscores = return_notch_bg_scores(jurkat_score_file)
            s,p = stats.ttest_rel(a_zsocres,j_zscores)
            #print(a_zsocres,j_zscores);exit()
            print(basename,hic_normalization,s,p)
            figname = outdir+os.sep+'{}_{}_zscore.pdf'.format(hic_normalization,basename)
            plot_lists = [a_zsocres,j_zscores]
