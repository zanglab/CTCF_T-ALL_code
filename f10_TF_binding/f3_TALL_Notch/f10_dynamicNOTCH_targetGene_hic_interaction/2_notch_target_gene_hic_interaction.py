...
def process_df_get_hic_interaction(df,union_feature_df,interaction_file,outfile_name):
    ...
    outf.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format('ctcf_id','gene_id','chr','ctcf_pos','notch_pos','gene_tss','gene_notch_interaction','intra_domain_notch_all_interactions'))
    for gene_id in df.index:
        ctcf_index = df.loc[gene_id,'ctcf_id']
        chr = union_feature_df.loc[ctcf_index,'chr']
        ctcf_pos = union_feature_df.loc[ctcf_index,'mid_position']
        domain_left = union_feature_df.loc[ctcf_index,'domain_100k_1M_left']
        domain_right = union_feature_df.loc[ctcf_index,'domain_100k_1M_right']
        
        pos1 = df.loc[gene_id,'hicor_notch_pos']
        #overlap_info = df.loc[id_index,'if_intra_domain_notch']
        #if overlap_info==1 and id_index in interaction_df.index:
        if 1:
            # get all 2m interaction score of this binding site
            interactions_2m_list = interaction_df.loc[ctcf_index]
            ...
            outf.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(ctcf_index,gene_id,chr,ctcf_pos,pos1,pos2,tmp_score,bg_scores))
...
def main():
    ...
    outdir = 'f2_Notch_gene_hic_interaction'
    ...
    ctcf_notch_gene_info_file = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/updated_201906/f10_TF_binding/f3_TALL_Notch/f6_CTCF_dynamic_notch_targetGene/f7_ctcf_notch_gene_figs/T_ALL_gained_CTCF_NOTCH_gene_master_new.csv'
    ctcf_notch_gene_info = pd.read_csv(ctcf_notch_gene_info_file,index_col=0)
    ctcf_notch_gene_info.index = ctcf_notch_gene_info['gene_id']
#     print(ctcf_notch_gene_info,ctcf_notch_gene_info.columns);exit()    
#     genes_file = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/updated_201906/f10_TF_binding/f3_TALL_Notch/f9_dynamicNOTCH_gained_CTCF_gene_expr/f3_dNOTCH_gainedCTCF_expr_in_shCTCF/cutll1_upgenes_dNotch_hic_increased_gained_ctcf_intra_domain_allgenes.txt'
#     genes = [ii.strip() for ii in genes_file.readlines()]

    basename = 'dynamicNOTCH_gainedCTCF_intra_domain_TALL_up_genes'
    process_df_get_hic_interaction(ctcf_notch_gene_info,union_feature_df,a6010_raw_file,outdir+os.sep+'{}_gene_notch_interaction_vs_bg_in_{}.csv'.format(basename,'a6010_raw'))
    process_df_get_hic_interaction(ctcf_notch_gene_info,union_feature_df,cutll1_raw_file,outdir+os.sep+'{}_gene_notch_interaction_vs_bg_in_{}.csv'.format(basename,'cutll1_raw'))
