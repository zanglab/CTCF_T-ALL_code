...
def mark_pvalue(compr_pos,positions,box_vals,flag):
    ...
    if p<0.05:
        ...
        if compr_pos[2] == 't':
            ...
            plt.text((x1+x2)*.5, y*1.1, p_label, ha='center', color=col,fontsize=15)
        else:
...
def box_plot_for_zscores(plot_lists,figname,xticklabels=None,legends=None,flag=None):
    ...
    plt.ylabel('Hi-C (z-score) of\nNOTCH1 and target gene',fontsize=18)
...
def return_notch_bg_scores(score_file):
    ...
    for id_index in score_df.index:
        compr_score = score_df.loc[id_index,'gene_notch_interaction']
        bg_score = [float(i) for i in score_df.loc[id_index,'intra_domain_notch_all_interactions'].split(',')]
...
def main():
    ...
    for basename in ['dynamicNOTCH_gainedCTCF_intra_domain_TALL_up_genes']:
        for hic_normalization in ['raw']:
            a6010_score_file = 'f2_Notch_gene_hic_interaction/{}_gene_notch_interaction_vs_bg_in_a6010_{}.csv'.format(basename,hic_normalization)
            cutll1_score_file = 'f2_Notch_gene_hic_interaction/{}_gene_notch_interaction_vs_bg_in_cutll1_{}.csv'.format(basename,hic_normalization)          
            print(basename,os.path.isfile(a6010_score_file),os.path.isfile(cutll1_score_file))
            ...
            outf = open(outfile,'w')
            outf.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format('id','gene_id','chr','ctcf_pos','gene_pos','notch_pos','cd4_zscore','cutll1_zscore','delta_zscore'))
            
            for index in a_df.index:
                id = a_df.loc[index,'ctcf_id']
                gene_id = a_df.loc[index,'gene_id']
                chr = a_df.loc[index,'chr']
                ctcf_pos = a_df.loc[index,'ctcf_pos']
                gene_pos = a_df.loc[index,'gene_tss']
                notch_pos = a_df.loc[index,'notch_pos']
                a_zscore = a_zscores[index]
                j_zscore = j_zscores[index]
                outf.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(id,gene_id,chr,ctcf_pos,gene_pos,notch_pos,a_zscore,j_zscore,j_zscore-a_zscore))
            outf.close()    
    
            z_score_df = pd.read_csv(outfile,sep='\t',index_col=1)
            ...
            figname = outdir+os.sep+'{}_{}_zscore.png'.format(hic_normalization,basename)
            ...
            genes = [ii.strip() for ii in open(genes_file).readlines()]
            z_score_df = z_score_df.loc[genes]
            a_zscores,j_zscores = z_score_df['cd4_zscore'],z_score_df['cutll1_zscore']
            s,p = stats.ttest_rel(a_zscores,j_zscores)
            #print(a_zscores,j_zscores);exit()
            print(basename,hic_normalization,s,p)
            figname = outdir+os.sep+'{}_{}_zscore_ctcf_notch_hic_increased.png'.format(hic_normalization,basename)
            plot_lists = [a_zscores,j_zscores]
            xticklabels = ['T-cell','T-ALL']
            legends = ['ctcf w/ intra-domain','ctcf w/ notch']
            flag = '{}'.format(basename,hic_normalization)
            box_plot_for_zscores(plot_lists,figname,xticklabels = xticklabels,legends = legends,flag = flag)
            z_score_df.to_csv(outfile+'_ctcf_notch_hic_increased.txt',sep='\t')
