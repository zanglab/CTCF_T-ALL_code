...
def mark_p(p,position,val):
    if p<0.01:
        plt.text(position,val-2,'*',ha='center')
#     if 0.0001<=p<0.001:
...
#     c0,c1,c2,c3,c4,c5 = val_const
    xticklabels = ['NOTCH1','dyanmic NOTCH1','MYC','NOTCH1 & MYC','dynamic NOTCH1 & MYC']
    plt.figure(figsize=(5,3))
    for ii in np.arange(1,len(gs)): 
        plot_positions = [ii,ii+0.25,ii+0.5]
        plot_vals = [100*cs[ii]/cs[0],100*ls[ii]/ls[0],100*gs[ii]/gs[0]]
        g=plt.bar(plot_positions,plot_vals,width=0.2,color = ['lightgrey','k','firebrick'])
        s,p = stats.fisher_exact([[cs[0]-cs[ii],cs[ii]],[ls[0]-ls[ii],ls[ii]]]);print(xticklabels[ii-1],'lost:',s,p)
        mark_p(p,ii+0.25,100*ls[ii]/ls[0])
        s,p = stats.fisher_exact([[cs[0]-cs[ii],cs[ii]],[gs[0]-gs[ii],gs[ii]]]);print(xticklabels[ii-1],'gained:',s,p)
        mark_p(p,ii+0.5,100*gs[ii]/gs[0])
    plt.ylabel('CTCF-containing\n domains (%)',fontsize=15)
    legends = ['Control CTCF','T-ALL Lost','T-ALL Gained']
    plt.legend((g[0],g[1],g[2]),legends,fontsize=14, frameon=False,borderaxespad=0.1,labelspacing=.1,handletextpad=0.2)
    #plt.xlim([0.0,4.5])
    plt.ylim([0,85])
    #plt.axes().tick_params(axis='x',direction='out', length=3, width=.8, colors='black')
    sns.despine(offset=0, trim=False)
    plt.axes().set_xticks(np.arange(1,len(gs))+0.3)
    plt.axes().set_xticklabels(xticklabels,rotation=30,ha = 'right',fontsize=14)
    plt.axes().tick_params(axis='y',direction='out', length=0, width=.8, colors='black')
...
def main(infile):
    ...
    outdir = 'f3_notch_myc_enrichment_figs'
    ...
    figname = '{}/NOTCH1_MYC_enrichment_bar.pdf'.format(outdir)
