...
def mark_pvalue(compr_pos,positions,box_vals):
    ...
    if p<0.05 or np.isnan(p):
        if compr_pos[2] == 't':
            ...
            plt.text((x1+x2)*.5, y*1.05, p_label, ha='center', color=col,fontsize=18)
        else:
            plt.plot([x1*1.03, x1*1.03, x2*0.97, x2*0.97], [y2, y2*.91, y2*.91, y2], lw=1, c=col)
            plt.text((x1+x2)*.5, y2*.85, p_label, ha='center', va='top', color=col,fontsize=18)
...
#         scatter_x = np.random.normal(positions[position_id],0.05,len(data[position_id]))
#         plt.scatter(scatter_x,data[position_id],color=colors[position_id],s=12,zorder=0,alpha=0.99)

    for compr_pos in [[0,1,'t']]:
        mark_pvalue(compr_pos,positions,data)
    #plt.legend(borderaxespad=0.1,labelspacing=.1,fontsize=14)
    
    plt.ylabel('# intra-domain\nNOTCH binding',fontsize=18)
...
def main(outdir):
    ...
    outdir='f2_dynamic_notch_density_figs'
...
#         print(df.columns);exit()
        ...
        col_names = ['annotation','domain_100k_1M_left', 'domain_100k_1M_right','dynamic_notch_center']
