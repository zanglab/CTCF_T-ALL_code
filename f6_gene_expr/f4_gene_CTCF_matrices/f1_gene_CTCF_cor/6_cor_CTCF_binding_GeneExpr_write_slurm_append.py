...
def write_out_slurm(gene_id,outdir):
    
    with open('{}/{}.slurm'.format(outdir,gene_id),'w') as slurm_out:
...
def main():
    ...
    gene_collections = [i for i in gene_collections if i.startswith('C')]
    
    step=6
    for gene_id in gene_collections:
        csv_file = 'f7_cor_CTCF_binding_geneExpr/CTCF_gene_cor_csv/{}_cor.csv'.format(gene_id)
        if os.path.isfile(csv_file):
           csv_lines = read_write_file.get_lines(csv_file)
           if csv_lines != 688429:
               print(gene_id,'Not finished')#;exit(0)
               write_out_slurm(gene_id,outdir)
        if not os.path.isfile(csv_file):
            print(gene_id,'No File')
            write_out_slurm(gene_id,outdir)
