...
def plot_plt_ax2(x,plot_lists,plot_labels,figname):
    ...
    ax1.set_ylabel('Pairs with $R^2$>0.25 (%)',fontsize=20)
    ax1.set_ylim([1,12.5])
    
    ax2 = ax1.twinx()
    c = ax2.plot(x,plot_lists[2],label = plot_labels[2],color = 'coral',linewidth=3)
    ax2.set_ylabel('-log$_{{10}}$ ($P$ value)',fontsize=20)
    ax2.set_yscale('log')
    ax2.axhline(y=-np.log10(0.05),color='k',linestyle='--',linewidth=1)
    ax2.set_ylim([1,1000])
    ls = a+b+c
    labs = [l.get_label() for l in ls]
    ax1.legend(ls,labs,loc=2,bbox_to_anchor=[.135,1],fontsize=16,frameon=False,borderaxespad=0.1,labelspacing=.1,handletextpad=1,handlelength=1)
...
def main():
    ...
    indir = 'f7_intra_inter_hicor_sep_chr_domain_NoLimit'
