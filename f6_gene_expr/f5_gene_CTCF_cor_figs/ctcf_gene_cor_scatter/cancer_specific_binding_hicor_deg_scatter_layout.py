...
def scatter_plot_ctcf_gene(ctcf_id,gene_id,figname):    
    ...
    plt.text(.55,.03,'$R^2$ = {:.2f}'.format(r_value**2),fontsize=15,transform=plt.axes().transAxes)
    #plt.legend([n,c],['Tissue','Cell Line'],frameon=True,fontsize=10,loc=0) 
    plt.xlabel('CTCF binding (sqrt RPKM)',fontsize=17)
    plt.ylabel('gene expression (sqrt TPM)',fontsize=17)
