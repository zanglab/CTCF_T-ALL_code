...
def intra_domain_diff_gene_plot_nobreak(cancertypes,pos_list,neg_list,figname,ylim1,ylim2,xticklabels=None,legend=None,ylabels=None,title=None,legend_loc=None):
    ...
    colors = ['w','silver','w','k']
    fig,axes = plt.subplots(nrows=2,sharex=False,figsize=(7,4))
    ...
    for pos in poses:
        ...
        if p2<0.05:
            ...
            if p2<0.001:
                star_mark="**"
            axes[1].text(pos+0.6,100*d2_s/d2_t,star_mark,ha='center',va='top')
    
    if legend_loc=="lower right":
        axes[1].legend([m[0],m[1],m[2],m[3]],legend,frameon=False,fontsize=13,loc="lower right",borderaxespad=-2.2,labelspacing=.1,handlelength=1) 
    else:
        axes[0].legend([m[0],m[1],m[2],m[3]],legend,frameon=False,fontsize=13,loc="upper right",borderaxespad=-2.2,labelspacing=.1,handlelength=1) 
