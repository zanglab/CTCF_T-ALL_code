...
def intra_domain_diff_gene_plot_nobreak(cancertypes,pos_list,neg_list,figname,ylim1,ylim2,xticklabels=None,legend=None,ylabels=None,title=None,legend_loc=None):
    ...
    colors = ['w','silver','w','k']
    fig,axes = plt.subplots(nrows=2,sharex=False,figsize=(7,5))
    ...
    for pos in poses:
        ...
        m = axes[0].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a1_s/a1_t,100*b1_s/b1_t,100*c1_s/c1_t,100*d1_s/d1_t],width=0.16,color=colors,edgecolor = "k",lw=1)
        m[2].set_hatch('///')
        ...
        m = axes[1].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a2_s/a2_t,100*b2_s/b2_t,100*c2_s/c2_t,100*d2_s/d2_t],width=0.16 ,color=colors,edgecolor = "k",lw=1)
        m[2].set_hatch('///')
    ...
    if legend_loc=="lower right":
        axes[1].legend([m[0],m[1],m[2],m[3]],legend,frameon=False,fontsize=12,loc="lower right",borderaxespad=-2,labelspacing=.2,handlelength=1) 
    else:
        axes[0].legend([m[0],m[1],m[2],m[3]],legend,frameon=False,fontsize=12,loc="upper right",borderaxespad=-2,labelspacing=.2,handlelength=1) 
    
    # background grey color to separate diff cancers
    axes[0].bar([1.3,3.3,5.3],[ylim2]*3,width=1,color=['whitesmoke']*3,edgecolor = "w",lw=0,zorder=0,clip_on=False)
    axes[0].bar([1.3,3.3,5.3],[-1.3*ylim2]*3,width=1,color=['whitesmoke']*3,edgecolor = "w",lw=0,zorder=0,clip_on=False)
    ...
    axes[1].set_xlim([-0.3,5.9])
    axes[0].set_xlim([-0.3,5.9])
...
def main_sub(suf_name):
    ...
    outdir = 'f4_panCancer_intra_domain_deg_pattern_figs'+os.sep+suf_name
