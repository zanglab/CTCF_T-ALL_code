...
def identification_of_gained_bindings(df,cancertype):
    ...
    all_thre,normal_thre,cancer_thre,all_pthre,normal_pthre = int(all_total*0.2), int(normal_total*0.0), max(int(cancer_total*0.5),2), 0.01, .1
...
def cancer_specific_binding(suf_name):

    outdir = 'f3_cancer_specific_binding_{}'.format(suf_name)
    os.makedirs(outdir,exist_ok=True)
    
    constitutive_bindings = CTCF_TALL_modules_new.return_constitutive_df()
...
#     cancertypes=['AML']
    for cancertype in cancertypes: 
        par_dir='/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/updated_201906/f6_gene_expr/f7_panCancer_deg_figs/f3_FeatureCombinaiton_CancerSpecificBinding_local'
        all_feature_file = par_dir+os.sep+'f2_combined_sep_cancertype_{}/{}_binding_features.csv'.format(suf_name,cancertype)
        ...
        constitutive_df = df.loc[constitutive_bindings.index]
        ...
        constitutive_df.to_csv(outdir+os.sep+'{}_const.csv'.format(cancertype))
...
#         lost_df.to_csv(outdir+os.sep+'{}_lost.bed'.format(cancertype),sep='\t',header=None,index=False)
        ...
        print(suf_name, cancertype);print('#gained',gained_df.shape);print('#lost',lost_df.shape,'\n====')
#         exit()
...
#     suf_names = ['domain_GT100K_LT1M_log2FC0.585_padj1e-3']
    
    for suf_name in suf_names:
        cancer_specific_binding(suf_name)
