...
def intra_domain_diff_gene_plot_nobreak(cancertypes,pos_list,neg_list,figname,ylim1,ylim2,xticklabels=None,legend=None,ylabels=None,title=None,legend_loc=None):
    ...
    colors = ['w','silver','w','k']
    fig,axes = plt.subplots(nrows=2,sharex=False,figsize=(5.6,3.6))
    ...
    for pos in poses:
        ...
        m = axes[0].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a1_s/a1_t,100*b1_s/b1_t,100*c1_s/c1_t,100*d1_s/d1_t],width=0.17,color=colors,edgecolor = "k",lw=1)
        m[2].set_hatch('///')
        ...
        if p2<0.05 and s2<1:
            ...
            if p2<0.001:
                star_mark="**"
            axes[0].text(pos+0.6,100*d1_s/d1_t,star_mark,ha='center',va='center')

        a2_s,a2_t = neg_list[pos][0]
        b2_s,b2_t = neg_list[pos][1]
        c2_s,c2_t = neg_list[pos][2]
        d2_s,d2_t = neg_list[pos][3]
        m = axes[1].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a2_s/a2_t,100*b2_s/b2_t,100*c2_s/c2_t,100*d2_s/d2_t],width=0.17 ,color=colors,edgecolor = "k",lw=1)
        m[2].set_hatch('///')
        ...
        if p1<0.05 and s1<1:
            ...
            if p1<0.001:
                star_mark="**"
            axes[1].text(pos+0.2,100*b2_s/b2_t +2,star_mark,ha='center',va='top')
        #if p2<0.05 and 100*d2_s/d2_t<ylim2:
        if p2<0.05 and s2<1:
            star_mark="*"
            if p2<0.001:
                star_mark="**"
            axes[1].text(pos+0.6,100*d2_s/d2_t +2,star_mark,ha='center',va='top')
    
    if legend_loc=="lower right":
        axes[1].legend([m[0],m[1],m[2],m[3]],legend,frameon=False,fontsize=13,loc="lower right",bbox_to_anchor=(.95, .92),borderaxespad=-2.2,labelspacing=.1,handlelength=1) 
    else:
        axes[0].legend([m[0],m[1],m[2],m[3]],legend,frameon=False,fontsize=13,loc="upper right",bbox_to_anchor=(.95, .92),borderaxespad=-2.2,labelspacing=.1,handlelength=1) 
...
#     cancertypes=['T-ALL','AML','BRCA','CRC','LUAD','PRAD','PRAD_TissueAdded']
    gained_file = pardir+os.sep+'f5_cancer_specific_binding_TALL_patinet_{}'.format(suf_name)+os.sep+'{}_gained.csv'.format(cancertype)
    lost_file = pardir+os.sep+'f5_cancer_specific_binding_TALL_patinet_{}'.format(suf_name)+os.sep+'{}_lost.csv'.format(cancertype)
    const_file = pardir+os.sep+'f5_cancer_specific_binding_TALL_patinet_{}'.format(suf_name)+os.sep+'{}_const.csv'.format(cancertype)
...
def main_sub(suf_name):
    ...
    outdir = 'f5_TALL_patient_intra_domain_deg_pattern_figs'+os.sep+suf_name
    ...
    cancertypes_ticks=['T-ALL','T-ALL patient1','T-ALL patient2']
    ...
    figname = outdir+os.sep+'intra_domain_diff_gene_enrichment_lost_selected_ctrl_patient.pdf'
