...
def intra_domain_hicor_gene_plot(cancertypes,pos_list,plot_diff_list,figname,ylim1,ylim2,xticklabels=None,legend=None,ylabels=None,title=None):
    ...
    colors = ['w','silver','w','k']
    fig,axes = plt.subplots(nrows=2,sharex=False,figsize=(5.7,4.5))#,gridspec_kw = {'height_ratios':[2, 2]})
    fig.subplots_adjust(hspace=.39)
...
#         b0_s,b0_t = pos_list[0][1]# df-p-s, df-p-t,
        a0_s,a0_t = pos_list[pos][0]# const-p-s, const-p-t,
        b0_s,b0_t = pos_list[pos][2]# df-p-s, df-p-t,
       
        a1_s,a1_t = pos_list[pos][1]# const-p-s, const-p-t,
        b1_s,b1_t = pos_list[pos][3]# df-p-s, df-p-t,
#         m = axes[0].bar([pos,pos+0.4],[100*a1_s/a1_t,100*b1_s/b1_t],width=0.32,color=colors,edgecolor = "k",lw=1)
        m = axes[0].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a0_s/a0_t,100*a1_s/a1_t,100*b0_s/b0_t,100*b1_s/b1_t],width=0.16,color=colors,edgecolor = "k",lw=1)
        m[2].set_hatch('///')
        ...
        if p1<0.05 and s1>1:
            ...
            if p1<0.001:
                star_marker='**'
            axes[0].text(pos+0.2,100*a1_s/a1_t,star_marker,ha='center',va='center')
        if p2<0.05 and s2>1:
            star_marker='*'
            if p2<0.001:
                star_marker='**'
            axes[0].text(pos+0.6,100*b1_s/b1_t,star_marker,ha='center',va='center')

#         a0_up,a0_dn,a0_t = plot_diff_list[0][0]# const-p-s, const-p-t,
#         b0_up,b0_dn,b0_t = plot_diff_list[0][1]# df-p-s, df-p-t,
        a0_up,a0_dn,a0_t = plot_diff_list[pos][0]# const-p-s, const-p-t,
        b0_up,b0_dn,b0_t = plot_diff_list[pos][2]# df-p-s, df-p-t,

        a2_up,a2_dn,a2_t = plot_diff_list[pos][1]# const-p-s, const-p-t,
        b2_up,b2_dn,b2_t = plot_diff_list[pos][3]# df-p-s, df-p-t,
...
#         m1 = axes[1].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a0_up/a0_t,100*a2_up/a2_t,100*b0_up/b0_t,100*b2_up/b2_t],width=0.16,color=['w','red','w','red'],edgecolor = "k",lw=1)
#         m2 = axes[1].bar([pos,pos+0.2,pos+0.4,pos+0.6],[-100*a0_dn/a0_t,-100*a2_dn/a2_t,-100*b0_dn/b0_t,-100*b2_dn/b2_t],width=0.16,color=['w','blue','w','blue'],edgecolor = "k",lw=1)
        m1 = axes[1].bar([pos,pos+0.2,pos+0.4,pos+0.6],[100*a0_up/a0_t,100*a2_up/a2_t,100*b0_up/b0_t,100*b2_up/b2_t],width=0.16,color=colors,edgecolor = "k",lw=1)
        m2 = axes[1].bar([pos,pos+0.2,pos+0.4,pos+0.6],[-100*a0_dn/a0_t,-100*a2_dn/a2_t,-100*b0_dn/b0_t,-100*b2_dn/b2_t],width=0.16,color=colors,edgecolor = "k",lw=1)
        m1[2].set_hatch('///');m2[2].set_hatch('///')
        ...
        if p1<0.05 and s1>1:
            ...
            if p1<0.001:
                star_marker='**'
            axes[1].text(pos+0.2,100*a2_up/a2_t,star_marker,ha='center',va='center')
        if p2<0.05 and s2>1:
            star_marker='*'
            if p2<0.001:
                star_marker='**'
            axes[1].text(pos+0.6,100*b2_up/b2_t,star_marker,ha='center',va='center')
        if p3<0.05 and s3>1:
            star_marker='*'
            if p3<0.001:
                star_marker='**'
            axes[1].text(pos+0.2,-100*a2_dn/a2_t -5,star_marker,ha='center',va='top')
        if p4<0.05 and s4>1:
            star_marker='*'
            if p4<0.001:
                star_marker='**'
            axes[1].text(pos+0.6,-100*b2_dn/b2_t -5,star_marker,ha='center',va='top')
    legend=['Promoter ctrl','Promoter','Domain ctrl','Intra-domain']
    axes[0].legend([m[0],m[1],m[2],m[3]],legend,loc='upper left',bbox_to_anchor=(.01, 1.05),frameon=False,fontsize=13,borderaxespad=-.1,labelspacing=.1,handletextpad=0.2,handlelength=1) 
#     legend1 = axes[1].legend([m1[0],m1[1]],['Up ctrl','Up'] ,loc='upper left',bbox_to_anchor=(.9, 1.15),frameon=False,fontsize=11,borderaxespad=-.1,labelspacing=.1,handletextpad=0.2,handlelength=1) 
#     legend2 = axes[1].legend([m1[1],m2[1]],['Up','Down'] ,loc='lower left',bbox_to_anchor=(.01, -0.08),frameon=False,fontsize=13,borderaxespad=-.1,labelspacing=.1,handletextpad=0.2,handlelength=1) 
#     axes[1].add_artist(legend1)
    
    # background grey color to separate diff cancers
    axes[0].bar([1.3,3.3,5.3],[46]*3,width=1,color=['whitesmoke']*3,edgecolor = "w",lw=0,zorder=0,clip_on=False)
    axes[0].bar([1.3,3.3,5.3],[-65]*3,width=1,color=['whitesmoke']*3,edgecolor = "w",lw=0,zorder=0,clip_on=False)
    
    axes[0].set(xticks = poses+0.3,xticklabels = xticklabels,ylabel=ylabels[0])
    ...
    axes[1].set_yticklabels([100,0,100])
    axes[0].set_ylim([0,45])
    axes[1].set_xlim([-0.3,5.8])
    axes[0].set_xlim([-0.3,5.8])
...
def main_sub(suf_name):
    ...
    outdir = 'f7_panCancer_intra_domain_hicor_figs'+os.sep+suf_name
    ...
    for cancertype in cancertypes:
        ...
        intra_domain_hicor_gained_list.append([ctrl_p,treat_p,ctrl_d,treat_d])
        ctrl_diffp,treat_diffp,ctrl_diffd,treat_diffd = return_intra_hicorDiff_list(gained_df,const_df)
        intra_domain_hicorDiff_gained_list.append([ctrl_diffp,treat_diffp,ctrl_diffd,treat_diffd])
        
        ctrl2_p,treat_p,ctrl2_d,treat_d = return_intra_hicor_list(lost_df,const_df)
        intra_domain_hicor_lost_list.append([ctrl2_p,treat_p,ctrl2_d,treat_d])
        ctrl2_diffp,treat_diffp,ctrl2_diffd,treat_diffd = return_intra_hicorDiff_list(lost_df,const_df)
        intra_domain_hicorDiff_lost_list.append([ctrl2_diffp,treat_diffp,ctrl2_diffd,treat_diffd])
...
#     xticklabels= np.append('Control',cancertype_sticklabels)
    xticklabels= cancertype_sticklabels
    legend=['Promoter','Intra-domain']
    ylabels=['Highly correlated \nCTCF-gene pairs (%)','Down-genes  Up-genes \n (%)        (%) ']
