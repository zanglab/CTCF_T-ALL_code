...
def find_overlap_by_searching_start_pos(chroms,expand,mutation_df,union_CTCF,outfile):
    ...
    for chr in chroms:
        ...
        mutation_ref2alt = list(mutation_chr['ref_to_alt'].values) # mutation status/phenotype
        mutation_rate = list(mutation_chr['mutation_rate'].values)
        ...
        for ctcf_ii in np.arange(len(ctcf_list)):
            ...
            for jj in np.arange(s,e):
                ...
                mutation_event_rate[insert_pos]+=mutation_rate[jj]
            ...
            #print(search_phenotype_list)
#             if ctcf_ii%100==0:
#                 print(ctcf_ii)
    outf.close()
    outf2.close()
#     with open(outfile+'.count','w') as outf2:
#         for mutation_count in count_mutation_list:
#             outf2.write('{}\t'.format(mutation_count))
#         print(count_mutation_list)
    #exit()
...
#     indir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/15_TCGA_patient_data/mutation/panCancer_ICGC/data/re_format_icgc_mutation'
    indir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/15_TCGA_patient_data/mutation/panCancer_ICGC/data_new_201907/f3_re_format_icgc_mutation/'
    outdir = 'f1_mutation_around_CTCF'
    os.makedirs(outdir,exist_ok=True)
    
    chroms = CTCF_TALL_modules_new.return_chroms()
#     union_CTCF = pd.read_csv('/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/13_interval_distribution/f0_union_binding_files/union_binding_basic.csv',index_col=3)
    ...
    for cancertype in ['BRCA','COAD','LUAD','PRAD','AML']:
        infile = indir+os.sep+'{}_mutation.bed'.format(cancertype) 
        with open(infile) as inf:
            df = pd.read_csv(inf,sep='\t',index_col=None)
        df = df.drop_duplicates()
        df = df.sort_values(by=['chr','pos'])
        check_rank_list(chroms,df,'chr','pos');print(cancertype)
        for expand in [9,200]:
            outfile=outdir+os.sep+'{}_mutation_on_CTCF_expand{}.csv'.format(cancertype,expand)
            # get the mutation info on CTCF motif
            find_overlap_by_searching_start_pos(chroms,expand,df,union_CTCF,outfile);print(expand)
