...
#     union_CTCF = CTCF_TALL_modules_new.return_occupancy_filtered()
    ...
    for celltype in cancertypes:
        for expand in [9,200]:
            ...
            gained_file ='f2_mutation_on_cancer_specific_CTCF/{}_mutation_on_{}_{}_expand{}.mutation_rate.csv'.format(cancertype_mutation_matchness[celltype],celltype,'gained',expand)
            lost_file =  'f2_mutation_on_cancer_specific_CTCF/{}_mutation_on_{}_{}_expand{}.mutation_rate.csv'.format(cancertype_mutation_matchness[celltype],celltype,'lost',expand)
            union_file = 'f1_mutation_around_CTCF/{}_mutation_on_CTCF_expand{}.csv.mutation_event_rate.csv'.format(cancertype_mutation_matchness[celltype],expand)
            
            gained_mutation_rate = get_mutation_info(gained_file)
            lost_mutation_rate = get_mutation_info(lost_file)
            union_mutation_rate = get_mutation_info(union_file)
            
            percentage_df.loc['union'] = union_mutation_rate
            percentage_df.loc['lost'] = lost_mutation_rate
            percentage_df.loc['gained'] = gained_mutation_rate
