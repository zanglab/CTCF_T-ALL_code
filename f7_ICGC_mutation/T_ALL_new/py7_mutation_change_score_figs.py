...
def plot_mutation_changes(df,cancertype,outdir,flag):    
    ...
    for ii in np.arange(len(df.index)):
        ...
        if plot_bar>0:
            g1 = plt.barh(ii,plot_bar,color='purple',height=1.2,linewidth=0)
        elif plot_bar<0:
            g1 = plt.barh(ii,plot_bar,color='green',height=1.2,linewidth=0)
...
def main():
    ...
    outdir = 'f7_mutation_change_bar_TALL'
    ...
    for cancertype in ['T-ALL']:
        for patient_type in ['CUTLL1','Jurkat','PD9','PD30']:
            for binding_type in ['gained','lost']:
                mutation_change_file = 'f6_mutation_matrix_score/{}_{}.csv'.format(patient_type,binding_type)
                ...
                mutation_df = plot_mutation_changes(mutation_df,patient_type,outdir,binding_type)
