...
def main():
    ...
    for celltype in ['CUTLL1','Jurkat','PD30','PD9']:
        for expand in [9,200]:
            ...
            mutation_file = indir+os.sep+'{}_mutation_on_CTCF_expand{}.csv'.format(celltype,expand)
            mutation_event_rate_file = indir+os.sep+'{}_mutation_on_CTCF_expand{}.csv.mutation_event_rate.csv'.format(celltype,expand)
            ...
            gained_mutation.to_csv(outdir+os.sep+'{}_mutation_on_TALL_{}_expand{}.csv'.format(celltype,'gained',expand),sep='\t',index=False)
            # save the mutation rate file
            gained_mutation_rate = df2.loc[gained_df.index]
            gained_mutation_rate.to_csv(outdir+os.sep+'{}_mutation_on_TALL_{}_expand{}.mutation_rate.csv'.format(celltype,'gained',expand),sep='\t',index=True,header=None)

            
            lost_mutation = df.loc[lost_df.index]
            lost_mutation.insert(1,'id',lost_mutation.index)
            lost_mutation.to_csv(outdir+os.sep+'{}_mutation_on_TALL_{}_expand{}.csv'.format(celltype,'lost',expand),sep='\t',index=False)

            lost_mutation_rate = df2.loc[lost_df.index]
            lost_mutation_rate.to_csv(outdir+os.sep+'{}_mutation_on_TALL_{}_expand{}.mutation_rate.csv'.format(celltype,'lost',expand),sep='\t',index=True,header=None)
