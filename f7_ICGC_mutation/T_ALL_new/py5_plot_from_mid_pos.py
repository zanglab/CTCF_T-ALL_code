...
#         print(x-smooth_window-mid_pos,min(len(df.columns)-1-mid_pos,x+window+smooth_window-mid_pos))
#         print(max(0,mirror_x-window-smooth_window),mirror_x+smooth_window,'\n')
        ...
        y_list.append(df.loc[index,plot_col].mean())#;print(y_list,'\n')
#    g = plt.plot(x_range,y_list,color=color)
    g = plt.bar(x_range+window*pos_add,y_list,color=color,width=window*.25)
...
def composite_plot(df,figname,window,title):
    ...
    plt.legend([g1[0],g2[0],g3[0]],['Union','Lost','Gained'],loc='upper right',frameon=False,fontsize=18,borderaxespad=0.1,labelspacing=.1,handlelength=1,handletextpad=0.2)
    ...
    plt.ylim([0,0.0025])
    if title =='PD9':
        title ='T-ALL patient1'
    if title =='PD30':
        title ='T-ALL patient2'
    plt.title(title)
...
def main():
    ...
    for celltype in ['CUTLL1','Jurkat','PD30','PD9']:
        for expand in [200]:
            ...
            print('\nT-ALL cell line, expand {}'.format(expand))
            ...
            for window in [40]:
                if (window<expand) and (window > expand/6.0):
                    ...
                    composite_plot(percentage_df,figname,window,celltype)
